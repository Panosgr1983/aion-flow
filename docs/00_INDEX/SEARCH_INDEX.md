# SEARCH INDEX — AION Knowledge & Engineering System

**Generated:** 2026-07-29 (final)

| ID | Title | Path | Status | MMI | Tags |
|----|-------|------|--------|-----|------|
| 00_INDEX.CURRENT_STATE | CURRENT STATE — AION Flow | 00_INDEX/CURRENT_STATE.md | — | — |  |
| 00_INDEX.MASTER_INDEX | MASTER INDEX — AION Knowledge & Engineering S | 00_INDEX/MASTER_INDEX.md | — | — |  |
| 00_INDEX.NEXT_APPROVED_ACTION | NEXT APPROVED ACTION | 00_INDEX/NEXT_APPROVED_ACTION.md | — | — |  |
| 00_INDEX.SEARCH_INDEX | SEARCH INDEX — AION Knowledge & Engineering S | 00_INDEX/SEARCH_INDEX.md | — | — |  |
| 00_INDEX.START_HERE | START HERE — AION Knowledge & Engineering Sys | 00_INDEX/START_HERE.md | — | — |  |
| 00_SESSION.SESSION_OBJECTIVE | SESSION OBJECTIVE Protocol | 00_SESSION/SESSION_OBJECTIVE.md | — | — |  |
| platform.aion-dna | AION DNA | 01_PLATFORM/AION_DNA.md | current | — | dna, identity, platform |
| 01_PLATFORM.ARCHITECTURE | AION — Platform Architecture Overview | 01_PLATFORM/ARCHITECTURE.md | — | — |  |
| 01_PLATFORM.ENGINEERING_PRINCIPLES | AION Flow — Engineering Principles | 01_PLATFORM/ENGINEERING_PRINCIPLES.md | — | — | principles, architecture, incident, platform |
| 01_PLATFORM.CONTENT_ENGINE | AION Flow — Content Engine | 01_PLATFORM/CONTENT_ENGINE.md | — | — | content, pipeline, tiptap, rich-text, engine |
| 01_PLATFORM.DOCUMENTATION_METHODOLOGY | AION Flow — Documentation Change Methodology | 01_PLATFORM/DOCUMENTATION_METHODOLOGY.md | — | — | documentation, methodology, audit, governance |
| 01_PLATFORM.ENGINEERING_PRINCIPLES (v2) | Engineering Principles (6 foundations) | 01_PLATFORM/ENGINEERING_PRINCIPLES.md | updated | — | principles, architecture, incident, platform, documentation |
| 01_PLATFORM.ADR.ADR-007-content-pipeline | ADR-007: Rich Content Pipeline | 01_PLATFORM/ADR/ADR-007-content-pipeline.md | — | — | adr, content, pipeline, rendering |
| INCIDENTS.2026-07-29-rich-content-json-leakage | Incident Report: Homepage TipTap JSON Leakage | INCIDENTS/2026-07-29-rich-content-json-leakage.md | — | — | incident, regression, json-leakage, content |
| 01_PLATFORM.CHANGELOG | AION CMS — Changelog | 01_PLATFORM/CHANGELOG.md | — | — |  |
| 01_PLATFORM.CONSTITUTION | AION Flow Development Constitution | 01_PLATFORM/CONSTITUTION.md | — | — |  |
| 01_PLATFORM.CREDENTIAL_ABSTRACTION_LAYER | Credential Abstraction Layer (CAL) | 01_PLATFORM/CREDENTIAL_ABSTRACTION_LAYER.md | — | — |  |
| 01_PLATFORM.DATABASE | AION CMS — Database Schema | 01_PLATFORM/DATABASE.md | — | — |  |
| 01_PLATFORM.DECISIONS | AION CMS — Architecture Decision Records | 01_PLATFORM/DECISIONS.md | — | — |  |
| 01_PLATFORM.DEPLOYMENT | AION CMS — Deployment Guide | 01_PLATFORM/DEPLOYMENT.md | — | — |  |
| 01_PLATFORM.FEATURES | AION CMS — Features | 01_PLATFORM/FEATURES.md | — | — |  |
| 01_PLATFORM.KNOWN_ISSUES | AION CMS — Known Issues | 01_PLATFORM/KNOWN_ISSUES.md | — | — |  |
| 01_PLATFORM.MODULES | AION CMS — Module Architecture | 01_PLATFORM/MODULES.md | — | — |  |
| 01_PLATFORM.PERMISSIONS | AION — Permissions & Access Control | 01_PLATFORM/PERMISSIONS.md | — | — |  |
| 01_PLATFORM.ROADMAP | AION — Roadmap | 01_PLATFORM/ROADMAP.md | — | — |  |
| 01_PLATFORM.STATUS | AION Flow — Platform Status | 01_PLATFORM/STATUS.md | — | — |  |
| 01_PLATFORM.TECH_DEBT | AION CMS — Technical Debt | 01_PLATFORM/TECH_DEBT.md | — | — |  |
| 01_PLATFORM.TELEMETRY | AION — Telemetry & Usage Events | 01_PLATFORM/TELEMETRY.md | — | — |  |
| 02_TENANTS.VISIBILITY_MATRIX | AION Flow — Tenant Visibility Matrix | 02_TENANTS/VISIBILITY_MATRIX.md | — | — |  |
| 02_TENANTS.kolokotronis.CONTENT_MAPPING | Tenant Content Mapping: Νικόλας Κολοκοτρώνης | 02_TENANTS/kolokotronis/CONTENT_MAPPING.md | — | — |  |
| 02_TENANTS.kolokotronis.HISTORY | Kolokotronis — Development History | 02_TENANTS/kolokotronis/HISTORY.md | — | — |  |
| 02_TENANTS.kolokotronis.README | Tenant: Νικόλας Κολοκοτρώνης | 02_TENANTS/kolokotronis/README.md | — | — |  |
| 02_TENANTS.ktima-kareli.CONTENT_MAPPING | Tenant Content Mapping: Κτήμα Καρέλη | 02_TENANTS/ktima-kareli/CONTENT_MAPPING.md | — | — |  |
| 02_TENANTS.ktima-kareli.README | Tenant: Κτήμα Καρέλη | 02_TENANTS/ktima-kareli/README.md | — | — |  |
| module.akes | AKES Module | 03_MODULES/AKES/README.md | current | 88% | akes, knowledge, governance |
| module.blog | Blog Module | 03_MODULES/Blog/README.md | current | 75% | blog, cms |
| module.bookings | Bookings Module | 03_MODULES/Bookings/README.md | current | 94% | bookings, retreat |
| module.crm | CRM Module | 03_MODULES/CRM/README.md | current | 50% | crm, inbox, pipeline |
| 03_MODULES.Locale.ARCHITECTURE | Locale Module — Architecture | 03_MODULES/Locale/ARCHITECTURE.md | — | — |  |
| 03_MODULES.Locale.CMS | Locale Module — CMS Panel | 03_MODULES/Locale/CMS.md | — | — |  |
| 03_MODULES.Locale.DATABASE | Locale Module — Database | 03_MODULES/Locale/DATABASE.md | — | — |  |
| module.locale | Locale Module | 03_MODULES/Locale/README.md | current | 6% | localization, translation |
| 03_MODULES.Locale.WORKFLOW | Locale Module — Bilingual Workflow | 03_MODULES/Locale/WORKFLOW.md | — | — |  |
| 03_MODULES.Media.KNOWN_ISSUES | Media Module — Known Issues | 03_MODULES/Media/KNOWN_ISSUES.md | — | — |  |
| 03_MODULES.Media.METHODS | Media Module — Methods | 03_MODULES/Media/METHODS.md | — | — |  |
| module.media | Media Module | 03_MODULES/Media/README.md | current | 81% | media, upload, storage |
| 03_MODULES.Portfolio.COMPLETION_REPORT_v1 | Portfolio Module v1.0 — Completion Report | 03_MODULES/Portfolio/COMPLETION_REPORT_v1.md | — | — |  |
| 03_MODULES.Portfolio.INTEGRATION_PLAN | Artist Module Integration Plan — AION Flow | 03_MODULES/Portfolio/INTEGRATION_PLAN.md | — | — |  |
| module.portfolio | Portfolio Module | 03_MODULES/Portfolio/MASTER.md | current | 75% | portfolio, crud, gallery |
| module.portfolio | Portfolio Module | 03_MODULES/Portfolio/README.md | current | 75% | portfolio, crud, gallery |
| 03_MODULES.Portfolio.reports.week-01-biography | Week 1: Biography CRUD — Completion Report | 03_MODULES/Portfolio/reports/week-01-biography.md | — | — |  |
| 03_MODULES.Portfolio.reports.week-02-filmography | Week 2: Filmography CRUD — Completion Report | 03_MODULES/Portfolio/reports/week-02-filmography.md | — | — |  |
| 03_MODULES.Portfolio.reports.week-03-television | Week 3: Television CRUD — Completion Report | 03_MODULES/Portfolio/reports/week-03-television.md | — | — |  |
| 03_MODULES.Portfolio.reports.week-03-theatre | Week 3: Theatre CRUD — Completion Report | 03_MODULES/Portfolio/reports/week-03-theatre.md | — | — |  |
| 03_MODULES.Portfolio.reports.week-04-timeline | Week 4: Timeline CRUD — Completion Report | 03_MODULES/Portfolio/reports/week-04-timeline.md | — | — |  |
| 03_MODULES.Portfolio.reports.week-05-gallery | Week 5: Gallery CRUD — Completion Report | 03_MODULES/Portfolio/reports/week-05-gallery.md | — | — |  |
| 03_MODULES.Retreat.ARCHITECTURE | Retreat Module — Architecture | 03_MODULES/Retreat/ARCHITECTURE.md | — | — |  |
| 03_MODULES.Retreat.CHANGELOG | Retreat Module — Changelog | 03_MODULES/Retreat/CHANGELOG.md | — | — |  |
| 03_MODULES.Retreat.CMS | Retreat Module — CMS Panels | 03_MODULES/Retreat/CMS.md | — | — |  |
| 03_MODULES.Retreat.CONTENT_MAPPING | Tenant Content Mapping: Κτήμα Καρέλη | 03_MODULES/Retreat/CONTENT_MAPPING.md | — | — |  |
| 03_MODULES.Retreat.DATABASE | Retreat Module — Database | 03_MODULES/Retreat/DATABASE.md | — | — |  |
| 03_MODULES.Retreat.LABELS | Retreat Module — Label Mapping | 03_MODULES/Retreat/LABELS.md | — | — |  |
| 03_MODULES.Retreat.LESSONS_LEARNED | Retreat Module — Lessons Learned | 03_MODULES/Retreat/LESSONS_LEARNED.md | — | — |  |
| module.retreat | Retreat Module | 03_MODULES/Retreat/MASTER.md | current | 75% | retreat, wellness, crud |
| 03_MODULES.Retreat.MEDIA | Retreat Module — Media | 03_MODULES/Retreat/MEDIA.md | — | — |  |
| 03_MODULES.Retreat.QA | Retreat Module — QA | 03_MODULES/Retreat/QA.md | — | — |  |
| 03_MODULES.Retreat.README | Retreat Module — AION Flow | 03_MODULES/Retreat/README.md | — | — |  |
| 03_MODULES.Retreat.ROADMAP | Retreat Module — Roadmap | 03_MODULES/Retreat/ROADMAP.md | — | — |  |
| 03_MODULES.Retreat.WORKFLOW | Retreat Module — Editorial Workflow | 03_MODULES/Retreat/WORKFLOW.md | — | — |  |
| 03_MODULES.Retreat.reports.week-01-tenant-setup | Week 1: Tenant Setup + Experiences CRUD — Com | 03_MODULES/Retreat/reports/week-01-tenant-setup.md | — | — |  |
| 03_MODULES.Retreat.reports.week-02-workshops-events | Week 2: Workshops + Events CRUD — Completion  | 03_MODULES/Retreat/reports/week-02-workshops-events.md | — | — |  |
| 03_MODULES.Retreat.reports.week-03-faq-bookings | Week 3: FAQ CRUD + Bookings Manager — Complet | 03_MODULES/Retreat/reports/week-03-faq-bookings.md | — | — |  |
| 04_METHODS.Core.MODULE_MATURITY | Module Maturity Model (MMI) | 04_METHODS/Core/MODULE_MATURITY.md | — | — |  |
| method.relationship-engine | Relationship Engine | 04_METHODS/Core/RELATIONSHIP_ENGINE.md | current | — | relationships, graph, metadata, index |
| 04_METHODS.Core.TENANT_ISOLATION_CHECKLIST | Tenant Isolation Checklist | 04_METHODS/Core/TENANT_ISOLATION_CHECKLIST.md | — | — |  |
| method.core.crud-pattern | Multi-Entry CRUD Panel Pattern | 04_METHODS/Core/crud-pattern.md | Standard | — | crud, pattern, react |
| method.core.module-registration | Module Registry Self-Registration | 04_METHODS/Core/module-registration.md | Standard | — | module-registry, architecture |
| method.core.tenant-resolution | Effective Tenant ID Resolution | 04_METHODS/Core/tenant-resolution.md | Standard | — | multi-tenant, tenant-isolation, rls, pla |
| 06_PLAYBOOKS.CREATE_MODULE | Playbook: Create Module | 06_PLAYBOOKS/CREATE_MODULE.md | — | — |  |
| 08_METRICS.AGENT_PERFORMANCE | Agent Performance Metrics | 08_METRICS/AGENT_PERFORMANCE.md | — | — |  |
| 08_REFERENCE.CREDENTIALS_REGISTRY | Credentials Registry | 08_REFERENCE/CREDENTIALS_REGISTRY.md | — | — |  |
| 08_REFERENCE.PROJECTS_REGISTRY | Projects Registry | 08_REFERENCE/PROJECTS_REGISTRY.md | — | — |  |
| 09_AI_MEMORY.AI_MEMORY | AI_MEMORY — How to Think & Work | 09_AI_MEMORY/AI_MEMORY.md | — | — |  |
| 09_AI_MEMORY.MEMORY_UPDATE_PROTOCOL | AKES Memory Update Protocol | 09_AI_MEMORY/MEMORY_UPDATE_PROTOCOL.md | — | — |  |
| 09_AI_MEMORY.METHOD_TEMPLATE | Method Template | 09_AI_MEMORY/METHOD_TEMPLATE.md | — | — |  |
| 09_AI_MEMORY.SESSION_LOGS.2026-07-08-to-2026-07-12 | Session Log — 2026-07-08 to 2026-07-12 | 09_AI_MEMORY/SESSION_LOGS/2026-07-08-to-2026-07-12.md | — | — |  |
| 09_AI_MEMORY.SESSION_LOGS.2026-07-13 | Session Log: 2026-07-13 — AKES v1.5 Relations | 09_AI_MEMORY/SESSION_LOGS/2026-07-13.md | — | — |  |
| 09_AI_MEMORY.SESSION_LOGS.2026-07-29 | Session Log: 2026-07-29 — Announcement Taxono | 09_AI_MEMORY/SESSION_LOGS/2026-07-29.md | — | — |  |
| 09_AI_MEMORY.SESSION_LOGS.2026-07-29b | Session Log: 2026-07-29b — Service FAQ (Perfo | 09_AI_MEMORY/SESSION_LOGS/2026-07-29b.md | — | — |  |
| 09_AI_MEMORY.SESSION_LOGS.2026-07-29c | Session Log: 2026-07-29c — Rich Content Engin | 09_AI_MEMORY/SESSION_LOGS/2026-07-29c.md | — | — |  |
| 09_AI_MEMORY.SESSION_LOG_TEMPLATE | Session Log Template | 09_AI_MEMORY/SESSION_LOG_TEMPLATE.md | — | — |  |
| vision.aion-ecosystem | AION Ecosystem Vision | 10_VISION/AION_ECOSYSTEM.md | vision | — | ecosystem, vision, flŐW, akes, nexus, cl |
| vision.nexus | AION NEXUS Vision | 10_VISION/NEXUS_VISION.md | vision | — | nexus, ai, orchestrator, vision |
| vision.pulse | AION PULSE Vision | 10_VISION/PULSE_VISION.md | vision | — | pulse, telemetry, observability, vision |
| vision.readme | 10_VISION — Future State Documents | 10_VISION/README.md | vision | — | vision, future, strategy |
| AGENT_PERFORMANCE | AION Flow — Agent Performance Metrics | AGENT_PERFORMANCE.md | — | — |  |
| ARCHITECTURE | AION — Platform Architecture Overview | ARCHITECTURE.md | — | — |  |
| BACKUP | AION CMS — Backup & Disaster Recovery | BACKUP.md | — | — |  |
| BLUEPRINTS | AION CMS — Blueprint Registry | BLUEPRINTS.md | — | — |  |
| CHANGELOG | AION CMS — Changelog | CHANGELOG.md | — | — |  |
| CODING_STANDARDS | AION CMS — Coding Standards | CODING_STANDARDS.md | — | — |  |
| CONSTITUTION | AION Flow Development Constitution | CONSTITUTION.md | — | — |  |
| CONTENT_MAPPING_KOLOKOTRONIS | Tenant Content Mapping: Νικόλας Κολοκοτρώνης | CONTENT_MAPPING_KOLOKOTRONIS.md | — | — |  |
| CONTRIBUTING | AION CMS — Contributing Guide | CONTRIBUTING.md | — | — |  |
| COPYWRITING | AION CMS — Copywriting Guide | COPYWRITING.md | — | — |  |
| CREDENTIALS | AION Flow — Tenant Credentials | CREDENTIALS.md | — | — |  |
| DATABASE | AION CMS — Database Schema | DATABASE.md | — | — |  |
| DECISIONS | AION CMS — Architecture Decision Records | DECISIONS.md | — | — |  |
| DEPLOYMENT | AION CMS — Deployment Guide | DEPLOYMENT.md | — | — |  |
| DESIGN_PRINCIPLES | AION CMS — Design Principles | DESIGN_PRINCIPLES.md | — | — |  |
| FEATURES | AION CMS — Features | FEATURES.md | — | — |  |
| KNOWN_ISSUES | AION CMS — Known Issues | KNOWN_ISSUES.md | — | — |  |
| MANIFESTO | AION CMS — Engineering Manifesto | MANIFESTO.md | — | — |  |
| MASTER.ARCHITECTURE_MAP | AION — Architecture Map | MASTER/ARCHITECTURE_MAP.md | — | — |  |
| MASTER.CURRENT_STATE | AION — Current State | MASTER/CURRENT_STATE.md | — | — |  |
| MASTER.DEVELOPMENT_RULES | AION — Development Rules | MASTER/DEVELOPMENT_RULES.md | — | — |  |
| MASTER.PERMISSIONS_MATRIX | AION — Permissions Matrix | MASTER/PERMISSIONS_MATRIX.md | — | — |  |
| MASTER.PROJECT_MEMORY | AION — Project Memory | MASTER/PROJECT_MEMORY.md | — | — |  |
| MASTER.VISION | AION — Vision & Product Philosophy | MASTER/VISION.md | — | — |  |
| MODULES | AION CMS — Module Architecture | MODULES.md | — | — |  |
| PERMISSIONS | AION — Permissions & Access Control | PERMISSIONS.md | — | — |  |
| PLATFORM_STATUS | AION Flow — Platform Status | PLATFORM_STATUS.md | — | — |  |
| PRODUCT_PHILOSOPHY | AION CMS — Product Philosophy | PRODUCT_PHILOSOPHY.md | — | — |  |
| QA_CHECKLIST | AION Flow — QA Checklist | QA_CHECKLIST.md | — | — |  |
| README | AION — Documentation | README.md | — | — |  |
| RELEASE_CHECKLIST | AION Release Checklist | RELEASE_CHECKLIST.md | — | — |  |
| RELEASE_PROCESS | AION Flow — Release Process | RELEASE_PROCESS.md | — | — |  |
| ROADMAP | AION — Roadmap | ROADMAP.md | — | — |  |
| TECH_DEBT | AION CMS — Technical Debt | TECH_DEBT.md | — | — |  |
| TELEMETRY | AION — Telemetry & Usage Events | TELEMETRY.md | — | — |  |
| TENANT_VISIBILITY | AION Flow — Tenant Visibility Matrix | TENANT_VISIBILITY.md | — | — |  |
| VERSIONS | AION CMS — Version History & Conventions | VERSIONS.md | — | — |  |
| archive.01-Architecture | AION Flow — Architecture Diagram | archive/01-Architecture.md | — | — |  |
| archive.02-Database-Schema | AION Flow — Database Schema | archive/02-Database-Schema.md | — | — |  |
| archive.03-Deployment-Guide | AION Flow — Deployment Guide | archive/03-Deployment-Guide.md | — | — |  |
| archive.04-API-Reference | AION Flow — API & Edge Function Reference | archive/04-API-Reference.md | — | — |  |
| archive.05-CRM-User-Manual | AION Flow — CRM User Manual | archive/05-CRM-User-Manual.md | — | — |  |
| archive.AION-FLOW-COMPLETE-v0.2 | AION Flow — Πλήρης Τεκμηρίωση Πλατφόρμας | archive/AION-FLOW-COMPLETE-v0.2.md | — | — |  |
| archive.README | AION — Legacy Documentation | archive/README.md | — | — |  |
| archive.knowledge-audit.MIGRATION_MANIFEST | Migration Manifest — AKES v1 | archive/knowledge-audit/MIGRATION_MANIFEST.md | — | — |  |
| modules.locale.ARCHITECTURE | Locale Module — Architecture | modules/locale/ARCHITECTURE.md | — | — |  |
| modules.locale.CMS | Locale Module — CMS Panel | modules/locale/CMS.md | — | — |  |
| modules.locale.DATABASE | Locale Module — Database | modules/locale/DATABASE.md | — | — |  |
| modules.locale.README | Locale Module — AION Flow | modules/locale/README.md | — | — |  |
| modules.locale.WORKFLOW | Locale Module — Bilingual Workflow | modules/locale/WORKFLOW.md | — | — |  |
| modules.portfolio.COMPLETION_REPORT_v1 | Portfolio Module v1.0 — Completion Report | modules/portfolio/COMPLETION_REPORT_v1.md | — | — |  |
| modules.portfolio.INTEGRATION_PLAN | Artist Module Integration Plan — AION Flow | modules/portfolio/INTEGRATION_PLAN.md | — | — |  |
| modules.portfolio.MASTER | Portfolio Module — MASTER Reference | modules/portfolio/MASTER.md | — | — |  |
| modules.portfolio.reports.week-01-biography | Week 1: Biography CRUD — Completion Report | modules/portfolio/reports/week-01-biography.md | — | — |  |
| modules.portfolio.reports.week-02-filmography | Week 2: Filmography CRUD — Completion Report | modules/portfolio/reports/week-02-filmography.md | — | — |  |
| modules.portfolio.reports.week-03-television | Week 3: Television CRUD — Completion Report | modules/portfolio/reports/week-03-television.md | — | — |  |
| modules.portfolio.reports.week-03-theatre | Week 3: Theatre CRUD — Completion Report | modules/portfolio/reports/week-03-theatre.md | — | — |  |
| modules.portfolio.reports.week-04-timeline | Week 4: Timeline CRUD — Completion Report | modules/portfolio/reports/week-04-timeline.md | — | — |  |
| modules.portfolio.reports.week-05-gallery | Week 5: Gallery CRUD — Completion Report | modules/portfolio/reports/week-05-gallery.md | — | — |  |
| modules.retreat.ARCHITECTURE | Retreat Module — Architecture | modules/retreat/ARCHITECTURE.md | — | — |  |
| modules.retreat.CHANGELOG | Retreat Module — Changelog | modules/retreat/CHANGELOG.md | — | — |  |
| modules.retreat.CMS | Retreat Module — CMS Panels | modules/retreat/CMS.md | — | — |  |
| modules.retreat.CONTENT_MAPPING | Tenant Content Mapping: Κτήμα Καρέλη | modules/retreat/CONTENT_MAPPING.md | — | — |  |
| modules.retreat.DATABASE | Retreat Module — Database | modules/retreat/DATABASE.md | — | — |  |
| modules.retreat.LABELS | Retreat Module — Label Mapping | modules/retreat/LABELS.md | — | — |  |
| modules.retreat.LESSONS_LEARNED | Retreat Module — Lessons Learned | modules/retreat/LESSONS_LEARNED.md | — | — |  |
| modules.retreat.MASTER | Retreat Module — MASTER Reference | modules/retreat/MASTER.md | — | — |  |
| modules.retreat.MEDIA | Retreat Module — Media | modules/retreat/MEDIA.md | — | — |  |
| modules.retreat.QA | Retreat Module — QA | modules/retreat/QA.md | — | — |  |
| modules.retreat.README | Retreat Module — AION Flow | modules/retreat/README.md | — | — |  |
| modules.retreat.ROADMAP | Retreat Module — Roadmap | modules/retreat/ROADMAP.md | — | — |  |
| modules.retreat.WORKFLOW | Retreat Module — Editorial Workflow | modules/retreat/WORKFLOW.md | — | — |  |
| modules.retreat.reports.week-01-tenant-setup | Week 1: Tenant Setup + Experiences CRUD — Com | modules/retreat/reports/week-01-tenant-setup.md | — | — |  |
| modules.retreat.reports.week-02-workshops-events | Week 2: Workshops + Events CRUD — Completion  | modules/retreat/reports/week-02-workshops-events.md | — | — |  |
| modules.retreat.reports.week-03-faq-bookings | Week 3: FAQ CRUD + Bookings Manager — Complet | modules/retreat/reports/week-03-faq-bookings.md | — | — |  |
| patterns.booking-pipeline | Booking Pipeline — AION Flow | patterns/booking-pipeline.md | — | — |  |
| patterns.client-approval | Client Approval Workflow — AION Flow | patterns/client-approval.md | — | — |  |
| patterns.documentation-process | Documentation Process — AION Flow | patterns/documentation-process.md | — | — |  |
| patterns.editorial-review | Editorial Review Process — AION Flow | patterns/editorial-review.md | — | — |  |
| patterns.external-project-setup | External Project Setup — AION Flow | patterns/external-project-setup.md | — | — |  |
| patterns.feature-flags | Feature Flags — AION Flow | patterns/feature-flags.md | — | — |  |
| patterns.gallery-pattern | Gallery Pattern — AION Flow | patterns/gallery-pattern.md | — | — |  |
| patterns.locale-module | Locale Module — AION Flow | patterns/locale-module.md | — | — |  |
| patterns.media-pipeline | Media Engine — AION Flow | patterns/media-pipeline.md | — | — |  |
| patterns.module-registry | Module Registry — AION Flow | patterns/module-registry.md | — | — |  |
| patterns.multi-project-pattern | Multi-Project Pattern — AION Flow | patterns/multi-project-pattern.md | — | — |  |
| patterns.portfolio-pattern | Portfolio Pattern — AION Flow | patterns/portfolio-pattern.md | — | — |  |
| patterns.research-workflow | Research Workflow — AION Flow | patterns/research-workflow.md | — | — |  |
| patterns.retreat-module | Retreat Module — AION Flow | patterns/retreat-module.md | — | — |  |
| patterns.tenant-isolation | Tenant Isolation — AION Flow | patterns/tenant-isolation.md | — | — |  |
| patterns.timeline-pattern | Timeline Pattern — AION Flow | patterns/timeline-pattern.md | — | — |  |
| releases.2026-07-29.RELEASE | Release — 2026-07-29 | releases/2026-07-29/RELEASE.md | — | — |  |
| releases.2026-07-29b.RELEASE | Release — 2026-07-29b | releases/2026-07-29b/RELEASE.md | — | — |  |
| releases.2026-07-29d.RELEASE | Release — 2026-07-29d | releases/2026-07-29d/RELEASE.md | — | — |  |
