...
const EMPTY = { title: '', description: '', duration: '', group_size: '', includes: [] as string[], image_url: '', sort_order: 0, status: 'draft' as string };

export default function WorkshopsCRUD() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    const { data } = await withTenant(supabase.from('workshops').select('*').order('sort_order', { ascending: true }), effectiveTenantId);
  ...
  const startEdit = (e: any) => { setForm({ title: e.title, description: e.description || '', duration: e.duration || '', group_size: e.group_size || '', includes: e.includes || [], image_url: e.image_url || '', sort_order: e.sort_order || 0, status: e.status || 'draft' }); setEditing(e.id); setError(null); };
  const cancel = () => { setEditing(null); setForm({ ...EMPTY }); setError(null); };

  const addInclude = () => { if (!includesInput.trim()) return; setForm(p => ({ ...p, includes: [...p.includes, includesInput.trim()] })); setIncludesInput(''); };
  ...
  const handleSave = async () => {
    ...
    const payload = { tenant_id: effectiveTenantId, title: form.title.trim(), description: form.description || null, duration: form.duration || null, group_size: form.group_size || null, includes: form.includes, image_url: form.image_url || null, sort_order: form.sort_order || 0, status: form.status || 'draft' };

    try {
      if (editing === 'new') {
        const { data: c, error: ie } = await supabase.from('workshops').insert(payload).select().single();
        if (ie) throw new Error(ie.message);
        await supabase.from('content_history').insert({ tenant_id: effectiveTenantId, table_name: 'workshops', record_id: c.id, entity_name: c.title, operation: 'create', snapshot_before: null, snapshot_after: { title: c.title }, summary: `Δημιουργία workshop: ${c.title}`, user_id: user?.id || null });
      } else {
        const before = items.find(e => e.id === editing);
        const { data: u, error: ue } = await supabase.from('workshops').update(payload).eq('id', editing).eq('tenant_id', effectiveTenantId).select().single();
        if (ue) throw new Error(ue.message);
        await supabase.from('content_history').insert({ tenant_id: effectiveTenantId, table_name: 'workshops', record_id: editing, entity_name: u.title, operation: 'update', snapshot_before: { title: before?.title }, snapshot_after: { title: u.title }, summary: `Ενημέρωση workshop: ${u.title}`, user_id: user?.id || null });
      }
      await load();
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Διαγραφή workshop; Η ενέργεια δεν μπορεί να αναιρεθεί.')) return;
    setDeleting(id);
    const { data: { user } } = await supabase.auth.getUser();
    const entry = items.find(e => e.id === id);
    await supabase.from('workshops').delete().eq('id', id).eq('tenant_id', effectiveTenantId);
    await supabase.from('content_history').insert({ tenant_id: effectiveTenantId, table_name: 'workshops', record_id: id, entity_name: entry?.title, operation: 'delete', snapshot_before: { title: entry?.title }, snapshot_after: null, summary: `Διαγραφή workshop: ${entry?.title}`, user_id: user?.id || null });
  ...
  if (editing) return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-white">{editing === 'new' ? 'Νέο workshop' : 'Επεξεργασία'}</h2>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-lg border border-gray-800 bg-gray-900/30 p-4 space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5"><label className="text-xs text-gray-500">Διάρκεια</label><input value={form.duration} onChange={e => updateForm('duration', e.target.value)} className={inputCls} /></div>
              <div className="space-y-1.5"><label className="text-xs text-gray-500">Μέγεθος ομάδας</label><input value={form.group_size} onChange={e => updateForm('group_size', e.target.value)} className={inputCls} placeholder="π.χ. 6-14 άτομα" /></div>
          ...
          <div className="rounded-lg border border-gray-800 bg-gray-900/30 p-4 space-y-3">
            ...
            <div className="flex gap-2"><input value={includesInput} onChange={e => setIncludesInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addInclude())} className={inputCls} placeholder="Προσθέστε στοιχείο..." /><button onClick={addInclude} className="px-3 py-2 text-xs bg-gray-700 text-white rounded-lg hover:bg-gray-600">+</button></div>
            <div className="flex flex-wrap gap-2">{form.includes.map((item, i) => (<span key={i} className="flex items-center gap-1 rounded-full bg-gray-800 px-3 py-1 text-xs text-gray-300">{item}<button onClick={() => removeInclude(i)} className="text-gray-500 hover:text-red-400 ml-1">×</button></span>))}</div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h2 className="text-lg font-semibold text-white">Workshops</h2><p className="text-sm text-gray-500">Διαχείριση εργαστηρίων & σεμιναρίων</p></div>
        <button onClick={startNew} className="flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-2 text-xs font-medium text-white hover:bg-blue-500"><Plus size={14} /> Νέο workshop</button>
      </div>
      {items.length === 0 ? (
        <div className="rounded-lg border border-gray-800 bg-gray-900/30 p-12 text-center"><p className="text-sm text-gray-500">Δεν υπάρχουν καταχωρημένα workshops.</p><button onClick={startNew} className="mt-4 text-xs text-blue-400">+ Προσθέστε το πρώτο</button></div>
      ) : (
          ...
          <div key={e.id} className="flex items-center justify-between rounded-lg border border-gray-800 bg-gray-900/30 px-4 py-3 hover:border-gray-700 group">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2"><span className="text-sm font-medium text-white truncate">{e.title}</span>{e.duration && <span className="text-[10px] text-gray-500">· {e.duration}</span>}</div>
                {e.group_size && <p className="text-xs text-gray-500">{e.group_size}</p>}
              </div>
            </div>
