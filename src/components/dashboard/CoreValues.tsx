...
import { coreValuesHelper } from '../../lib/dataHelpers';
import { CoreValue } from '../../types/supabase';

const emptyForm: Partial<CoreValue> = { tenant_id: null, title: '', description: '', icon: 'heart', sort_order: 0, is_active: true };
...
export default function CoreValues() {
  ...
  useEffect(() => { coreValuesHelper.getAll().then(d => { setItems(d); setLoading(false); }); }, []);

  const filtered = items.filter(i => i.title.toLowerCase().includes(search.toLowerCase()));
  const openAdd = () => { setEditing(null); setForm({ ...emptyForm, sort_order: items.length + 1 }); setShowModal(true); };
  const openEdit = (item: CoreValue) => { setEditing(item); setForm(item); setShowModal(true); };

  const handleSave = async () => {
    setSaving(true);
    if (editing) { const updated = await coreValuesHelper.update(editing.id, form); setItems(prev => prev.map(i => i.id === editing.id ? updated : i)); }
    else { const created = await coreValuesHelper.create(form); setItems(prev => [...prev, created]); }
    setSaving(false); setShowModal(false);
  };

  const handleDelete = async () => {
    if (!deleteId) return; await coreValuesHelper.delete(deleteId); setItems(prev => prev.filter(i => i.id !== deleteId)); setDeleteId(null);
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div><h2 className="text-xl font-semibold">Αξίες</h2><p className="text-sm text-gray-500">{items.length} αξίες</p></div>
        <button onClick={openAdd} className="btn-primary"><Plus size={16} /> Νέα Αξία</button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map(item => (
          <div key={item.id} className="card p-4 hover:border-gray-700 transition-colors">
            <div className="flex items-start justify-between mb-3">
              <div className="w-10 h-10 bg-rose-500/10 rounded-xl flex items-center justify-center"><Heart size={18} className="text-rose-400" /></div>
        ...
        {filtered.length === 0 && <div className="col-span-full text-center py-12 text-gray-500">Δεν βρέθηκαν αξίες</div>}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 w-full max-w-lg" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold mb-4">{editing ? 'Επεξεργασία' : 'Νέα'} Αξία</h3>
