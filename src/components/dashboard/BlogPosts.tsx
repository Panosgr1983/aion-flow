...
const emptyForm: Partial<BlogPost> = { tenant_id: null, title: '', slug: '', excerpt: '', content: {}, category: '', image_url: '', is_published: false, published_at: null, meta_title: '', meta_description: '', og_image: '', show_on_service_page: false, show_date: false };
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 w-full max-w-3xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                <div><label className="text-xs text-gray-500 block mb-1">Σελίδα υπηρεσίας</label><label className="flex items-center gap-2 mt-1 cursor-pointer"><input type="checkbox" checked={form.show_on_service_page || false} onChange={e => setForm(f => ({ ...f, show_on_service_page: e.target.checked }))} className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-blue-500 focus:ring-blue-500/30" /><span className="text-sm text-gray-400">Εμφάνιση στη σελίδα υπηρεσίας</span></label></div>
                <div><label className="text-xs text-gray-500 block mb-1">Ημερομηνία</label><label className="flex items-center gap-2 mt-1 cursor-pointer"><input type="checkbox" checked={form.show_date || false} onChange={e => setForm(f => ({ ...f, show_date: e.target.checked }))} className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-blue-500 focus:ring-blue-500/30" /><span className="text-sm text-gray-400">Εμφάνιση ημερομηνίας δημοσίευσης</span></label></div>
              </div>
